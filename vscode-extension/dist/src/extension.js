...
class InklinkPanel {
    ...
    static createOrShow(extensionUri, fileUri, context) {
        ...
        if (InklinkPanel.currentPanel) {
            InklinkPanel.currentPanel._panel.reveal(column);
            return;
        }
        const panel = vscode.window.createWebviewPanel('inklinkMap', `Inklink: ${path.basename(fileUri.fsPath)}`, column || vscode.ViewColumn.One, {
            enableScripts: true,
