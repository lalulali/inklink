...
export function SettingsDialog() {
	...
	return (
		<Sheet open={isOpen} onOpenChange={(open) => !open && handleClose()}>
			<SheetContent
				side="right"
				className="w-[400px] sm:w-[540px] sm:max-w-md overflow-y-auto bg-background flex flex-col p-6 border-l border-border"
			>
				<SheetHeader>
					...
					<div className="flex flex-col gap-4 p-5 rounded-2xl bg-primary/5 border border-primary/10 transition-all hover:bg-primary/[0.08] group/ext shadow-sm hover:shadow-md">
						...
						<div className="grid grid-cols-2 gap-3 mt-1">
							<Button
								variant="outline"
								size="sm"
								className="h-10 gap-2 text-[9px] font-black uppercase tracking-[0.15em] border-primary/20 bg-background/50 hover:bg-primary/10 hover:border-primary/40 transition-all shadow-sm active:scale-95"
								asChild
							>
								<a
				...
				<div className="flex justify-between items-center pt-6 mt-4 border-t border-border/50">
					<Button
						variant="ghost"
						size="sm"
						className="h-9 gap-2 text-muted-foreground hover:text-primary transition-colors font-bold text-[10px] uppercase tracking-widest"
						asChild
					>
						<a
